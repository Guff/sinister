...
from sinister.cplottable cimport Plottable
from sinister.config import conf

from gi.repository import Gdk
import cairo

cdef class FunctionPlot(Plottable):
    cdef public object func
    cdef public object color
    
    def __init__(self, viewport, func):
        super(FunctionPlot, self).__init__(viewport)
    ...
    def __call__(self, window_x):
        plot_x = self.window_to_plot(window_x, None)
        try:
            plot_y = self.func(plot_x)
        except:
            return None
        else:
            window_y = self.plot_to_window(None, plot_y)
            return window_y

cdef class PlotBg(Plottable):
    cdef public object ticks
    
    def __init__(self, viewport, ticks=(1.0, 1.0)):
        super(PlotBg, self).__init__(viewport)
